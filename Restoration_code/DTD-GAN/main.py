...
def main():
    ...
    parser.add_argument('--dim', default=16, type=int, help='initial feature dimension (doubled at each downsampling layer)')
#For the final model --dim is 16 and baseline dim 32 #################################################################
    ...
    parser.add_argument('--weight-Z-Adv', default=0.1, type=float, help='weight of adversarial loss after color net')
...
#    test_loader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, num_workers=args.workers, pin_memory=False, shuffle=False)
    ...
    print(model)
    model = model.cuda()	
    model.load_state_dict(torch.load('/home/shyam.nandan/TurbulentWater-master_v2/results/test_net.pth'))
...
def train(loader,vloader, model, epoch, args, BestPSNR):
    ...
    for i, ((input, target), _) in enumerate(loader):
		...
		inp = input.data.cpu().numpy().reshape((224, 224, 3))
