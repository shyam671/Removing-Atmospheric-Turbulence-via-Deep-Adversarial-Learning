...
class ResidualGroup(nn.Module):
    def __init__(self, dim, norm='in', activation='relu', pad_type='zero', n_resblocks = 2):
        super(ResidualGroup, self).__init__()
        model = []
        model = [
 #           RCAB(
 #              conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.ReLU(True), res_scale=1) \
             ResBlockCAB(dim, norm=norm, activation=activation, pad_type=pad_type) \
            for _ in range(n_resblocks)]

	model.append(Conv2dBlock(dim ,dim, 3, 1, 1, norm='none', activation='none', pad_type=pad_type))
...
class Conv2dBlock(nn.Module):
    def __init__(self, input_dim ,output_dim, kernel_size, stride,
        ...
        if norm == 'bn':
            ...
            self.norm = nn.GroupNorm(norm_dim/16, norm_dim)
        elif norm == 'none':
            self.norm = None
        else:
