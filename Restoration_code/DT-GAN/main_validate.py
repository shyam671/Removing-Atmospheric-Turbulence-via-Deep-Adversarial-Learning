...
import networks_basline1 as networks
...
#    test_loader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, num_workers=args.workers, pin_memory=False, shuffle=False)
    ...
    print(model)
    model.cuda()
    model.load_state_dict(torch.load('/home/shyam.nandan/WGAN/LearningToSeeThroughTurbulrntWater/TurbulentWater-master_v1/results/best.pth'))
...
def train(loader, model, epoch, args):
	...
	torchvision.utils.save_image(z.cpu(), os.path.join(args.outroot,'BSL1',ipath[0][5:]), nrow=1, normalize=True, range=(-1,1)) 
	print(i)
