...
#    test_loader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, num_workers=args.workers, pin_memory=False, shuffle=False)
    ...
    print(model)
    model.cuda()
    model.load_state_dict(torch.load('/home/shyam.nandan/Air_Exp/TurbulentWater-Enssemble/results/best.pth'))
...
def train(loader, model, epoch, args):
	...
	## Average
        ...
        z = (z_0 + z_90f + z_90 + z_270 + z_270f + z_180f )/6
        #z = z_0
	##############################################################################################
        ##PSNR and MSE for restored images
	t = target.data.cpu().numpy().reshape((224, 224, 3))
	p = z.data.cpu().numpy().reshape((224, 224, 3))
	torchvision.utils.save_image(z.cpu(), os.path.join(args.outroot,'Enssemble_FinalModel_DTD_GAN',ipath[0][5:]), nrow=1, normalize=True, range=(-1,1))
