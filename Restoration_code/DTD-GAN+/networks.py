...
#        color corrector network (used to correct color shift between monitor images and real images in the training set)
        ...
        # warp net
        if args.warp_net:
            self.warp_net = I2INetRDN(3, 2, args.warp_net_downsample, False, args.dim, args.n_res, args.norm, 'relu', 'reflect', final_activ='none')
        ...
        if args.color_net:
            ...
            self.color_net = I2INetRDN(3, 3, args.warp_net_downsample, args.color_net_skip, args.dim, args.n_res, args.norm, 'relu', 'reflect', args.denormalize)
...
######################change in lr for categorical###################################################
    ...
    def forward(self, w, cc=True):
        ...
        if self.warp_net is not None:
            x_up = torch.nn.functional.upsample(x, scale_factor=2, mode='bilinear', align_corners=True)
            warp = self.warp_net(x)*10
            y = Interp.warp(x_up,warp[:,0,:,:],warp[:,1,:,:])
            d = torch.nn.AvgPool2d(2, stride=2)
            y = d(y)
        else:
...
class I2INetRDN(nn.Module):
    def __init__(self, input_dim=3, output_dim=3, n_downsample=3, skip=True, dim=32, n_res=8, norm='in', activ='relu', pad_type='reflect', denormalize=False, final_activ='tanh'):
        ...
        for i in range(n_res):
            self.res_blocks.append(RDB(dim,dim, 8) )
	...
	if output_dim == 2:
            for i in range(n_downsample):
                self.up_blocks.append( nn.Sequential(nn.Upsample(scale_factor=2),Conv2dBlock(dim, dim // 2, 5, 1, 2, norm=norm, activation=activ, pad_type=pad_type)) )
                dim //= 2
	    # Subsampling Layer
            self.conv_out = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear'),Conv2dBlock(dim, output_dim, 7, 1, 3, norm='none', activation=final_activ, pad_type=pad_type))

	if output_dim == 3:
	    for i in range(n_downsample):
               self.up_blocks.append(nn.Sequential(Conv2dBlock(dim, dim*2, 5, 1, 2, norm=norm, activation=activ, pad_type=pad_type), nn.PixelShuffle(2)) )
               dim //= 2
...
class I2INet(nn.Module):
    def __init__(self, input_dim=3, output_dim=3, n_downsample=3, skip=True, dim=32, n_res=8, norm='in', activ='relu', pad_type='reflect', denormalize=False, final_activ='tanh'):
        ...
        self.res_blocks = nn.ModuleList()
        for i in range(n_res):
            self.res_blocks.append( ResidualGroup(dim, norm=norm, activation=activ, pad_type=pad_type) )
